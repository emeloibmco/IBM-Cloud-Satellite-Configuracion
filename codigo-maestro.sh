...
for ((i = 2; i <= cantidad_filas_control_plane; i++)); do
		# extracción de una fila en espécifico del archivo de HostSebas.txt 
...
for ((i = 2; i <= cantidad_filas_worker_nodes; i++)); do
	...
	#Extraer el nombre del host
	name=$(echo "$linea" | awk '{print $2}')
	ibmcloud sat host update --host $name --host-label nodos=workers --location $location
done 
...
ibmcloud oc cluster create satellite --location $location --name $nombreOpenshift --version 4.12.37_openshift --workers 3 --operating-system RHEL8 --enable-config-admin --host-label nodos=workers
